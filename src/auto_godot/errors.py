...
All auto-godot errors inherit from AutoGodotError and carry a machine-readable
...
@dataclass
class AutoGodotError(Exception):
    """Base exception for all auto-godot errors.
...
@dataclass
class ParseError(AutoGodotError):
    """Raised when a Godot file cannot be parsed."""


@dataclass
class ResourceNotFoundError(AutoGodotError):
    """Raised when a referenced file or resource does not exist."""


@dataclass
class GodotBinaryError(AutoGodotError):
    """Raised when the Godot binary is missing, wrong version, or fails."""


@dataclass
class ValidationError(AutoGodotError):
    """Raised when input data fails validation."""


@dataclass
class ProjectError(AutoGodotError):
