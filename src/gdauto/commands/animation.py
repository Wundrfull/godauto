...
from gdauto.formats.uid import generate_uid
from gdauto.formats.values import NodePath, SubResourceRef
...
def create_library(
    ...
    Examples:
      ...
      gdauto animation create-library --name idle --name walk --length 1.0 --length 0.8 --loop linear --loop linear animations.tres

      gdauto animation create-library --name attack --length 0.5 animations/combat.tres
    """
    try:
...
def add_track(
    ...
    Examples:
      ...
      gdauto animation add-track --library anims.tres --animation idle --property "Sprite2D:modulate:a" --keyframe "0=1.0" --keyframe "0.5=0.5" --keyframe "1.0=1.0"

      gdauto animation add-track --library anims.tres --animation walk --property ".:position:x" --keyframe "0=0" --keyframe "0.5=16" --keyframe "1.0=0"
    """
    try:
        ...
        from gdauto.formats.tres import parse_tres
        ...
        if target_sub is None:
            raise ProjectError(
                ...
                fix=f"Available animations: check with 'gdauto animation list-tracks'",
            )
